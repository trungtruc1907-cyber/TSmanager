...
export default function App() {
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 font-sans overflow-hidden relative">
      ...
      <header className="bg-white sticky top-0 z-40 shadow-sm backdrop-blur-md bg-white/90">
        <div className={cn(
          "h-16 lg:h-20 px-4 lg:px-8 flex items-center justify-between w-full",
          activeView === "warehouse" ? "border-b border-slate-100" : "border-b border-slate-200"
        )}>
              ...
              {warehouseTabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "px-3.5 py-2.5 text-[10px] lg:text-[11px] font-black uppercase tracking-wider flex items-center gap-1.5 border-t-2 border-x transition-all duration-200 cursor-pointer shrink-0",
                      isActive 
                        ? "bg-white border-t-blue-600 border-x-slate-200 text-blue-600 rounded-t-xl z-10 -mb-px shadow-xs" 
                        : "bg-transparent border-t-transparent border-x-transparent text-slate-400 hover:text-slate-600 hover:bg-slate-100/30 rounded-t-lg"
                    )}
                  >
